          ...
          <a href="{{ url('/mygym/'.$value['id']) }}">
            <div class="image-container">
              ...
              @foreach($gym_imagess as $index=>$image)
                @if($index==0)
                <img height="250" width="250" class="card-img-top" src="{{ asset('/gyms') }}/{{$image}}"  alt="{{$value['gym_name']}}"/>
                @endif
              @endforeach
            </div>
          </a>
                    ...
                    <div class='col-md-8 pt-3'> 
                      <div class='row'> 
                        @foreach($value->packages as $index=>$packages)
                          @if ($index <= 3)
                          <?php //$images = explode('|', $packages->image);  ?>
                            <div class='col-md-6'> 
                                <div class=''> 
                                    <div class='image-container'> 
                                        <img height='100' width='80' class='img-fluid' src='{{  asset('/package/'.$packages->image) }}/'/> 
                                    </div>
